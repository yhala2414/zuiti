...
- Regression coverage is provided by `npm run test:p2`, `npm run test:p1`, and `npm run test:p0`.
- Local route checks via `curl -I` returned 200 for `/`, `/input?scene=work&style=refuse`, `/tone`, and `/results`.
